...
export default function AboutFour() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative min-h-svh pb-30 w-full flex flex-col justify-center items-center z-20 overflow-hidden bg-zinc-900"
    >
      <div className="flex h-svh justify-center items-center w-full">
        <div className="overflow-hidden">
          <h3
            ref={titleRef}
            className="text-white text-8xl text-center px-20 font-black drop-shadow-[10px_10px_0px_#000]"
          >
            The principles behind how I design and build experiences
          </h3>
        </div>
      </div>
      <div className="w-full flex flex-col px-20 text-white gap-50">
        {item.map((item, idx) => {
          ...
          return (
            <div
              ...
              className={`w-full items-center flex ${isEven ? "justify-start" : "justify-end"}`}
            >
              <div className="h-fit relative max-w-lg">
                ...
                <div className="flex flex-col pl-5 gap-5">
                  ...
                  >
                    <h4 className="text-6xl text-zinc-100">{item.title}</h4>
                  </div>
                  <motion.span
                    initial={{
                      opacity: 0,
                    ...
                    className="inline-block text-base md:text-lg leading-relaxed font-medium text-zinc-100"
                  >
                    {item.subTitle}
                  </motion.span>
                </div>
